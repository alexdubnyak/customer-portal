...
if [ -f "package.json" ]; then
    # Проверяем, это Create React App или Vite
    if grep -q "react-scripts" package.json; then
        BUILD_DIR="build"  # Create React App
...
if [ -f "deploy.sh" ]; then
    ...
    echo "⚠️  deploy.sh не найден - создайте файл"
fi
...
if [ -f "package.json" ]; then
    ...
    # Для Create React App добавляем homepage
    if grep -q "react-scripts" package.json && ! grep -q '"homepage"' package.json; then
        sed -i.tmp "s|\"name\":.*|&\n  \"homepage\": \"/$PROJECT_NAME\",|" package.json
    fi
...
echo ""
echo "📋 Что дальше:"
echo "1. Проверьте package.json (homepage должен быть /$PROJECT_NAME)"
